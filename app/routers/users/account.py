...
async def read_users_me(user=Depends(get_current_user)):
    ...
    Args:
        user (UserInDB, optional): The user object of the currently logged-in user. Defaults to Depends(get_current_user).
        db (AsyncIOMotorClient, optional): The MongoDB database object. Defaults to Depends(get_db).
